...
class PlaywrightScreen(QWidget):
    ...
    def on_execution_finished(self, success: bool, message: str):
        ...
        self.add_test_result(
            PlaywrightTestResult(
                ...
                result=result,
                test_script=self.script_editor.toPlainText(),
    ...
    def add_test_result(self, result: PlaywrightTestResult):
        ...
        items = [
            ...
            result.expected_result, result.result, result.notes,result.test_script
        ]
    ...
    def copy_jira_comment(self):
        ...
        # Get data from table
        focus = self.report_table.item(row, 0).text() if self.report_table.item(row, 0) else "N/A"
        ...
        test_script = self.report_table.item(row, 9).text() if self.report_table.item(row, 9) else "N/A"
        notes = self.report_table.item(row, 8).text() if self.report_table.item(row, 8) else ""
        
        # Format comment
        comment = 'Hi, I have completed the testing and the results are as expected. Below are the modules I tested: \n'
        comment += f" - {focus} [{result}] \n"
        # if notes:
        #     comment += f" - {notes}"
        # if test_script:
        #     comment += f" - Test Script: {test_script}"
